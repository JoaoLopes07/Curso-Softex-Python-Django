...
def main():
    ...
    cursor.execute(
        "UPDATE Produtos SET preco = preco * 1.10 WHERE nome = 'Mouse Gamer'"
    )
    cursor.execute(
        "UPDATE Produtos SET estoque = estoque - 2 WHERE nome = 'Mouse Gamer'"
    )
    conexao.commit()
    ...
    cursor.execute(
        "DELETE FROM Itens_Pedido WHERE id_pedido = ? AND id_produto = 1", (id_pedido,)
    )
    conexao.commit()
